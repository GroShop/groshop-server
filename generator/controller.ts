// @ts-nocheck
import _MNS_Service from "../services/_MN_.service";
...
const _MNS_Controller = {
  create_MNS_: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      let _MN_ = orgService.getOrg({ domain: req.body._UP_ });
      if(_MN_) return res.status(HTTP.UNPROCESSABLE_ENTITY).send({ status: STATUS.FAILED, message: _MNC__RESPONSE.ALREADY_EXIST });
      const create_MNS_ = await _MNS_Service.create_MNS_(req.body);
      if (create_MNS_) {
        res.send({
  ...
  get_MNS_: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      ...
      const get_MNS_ = await _MNS_Service.get_MNS_({ _id: _MN__id });
  ...
  getMany_MNS_: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      ...
      const getMany_MNS_ = await _MNS_Service.getMany_MNS_WithPagination(query, { skip, limit });
      res.send({
        status: STATUS.SUCCESS,
  ...
  edit_MNS_: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      ...
      const edit_MNS_ = await _MNS_Service.edit_MNS_({ _id: _MN__id }, body);
      if (edit_MNS_) {
        const query = {
          _id: _MN__id,
        };
        const get_MNS_ = await _MNS_Service.get_MNS_(query);
        res.send({
          status: STATUS.SUCCESS,
  ...
  delete_MNS_: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      const { _MN__id } = req.body;
      const delete_MNS_ = await _MNS_Service.delete_MNS_({ _id: _MN__id });
      if (delete_MNS_) {
        res.send({
          status: STATUS.SUCCESS,
...
export default _MNS_Controller;
